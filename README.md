...
## start local node (using for pusher - temporary).

Should use local firebase functions but I get cors issues with that, so its just faster this way right now
```cd functions && node local`
